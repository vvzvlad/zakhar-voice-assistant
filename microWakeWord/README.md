...
- **v27** = v16-рецепт + **синтетические короткие-«захар» негативы** (лёгкий вес 4/5) — ПЕРВАЯ модель,
  которая **бьёт v16**: чинит полевой баг «палит на короткий захар» (firing@1.0с **65%→23%**) при
  чуть ЛУЧШЕМ recall (FRR 21→**19.3%** @0.90) и низком FAPH по всем классам (music 5.8→2.9/ч).
  Размер тот же (77848 B). Синтетику короткую заменим на РЕАЛЬНУЮ → дожмёт. Сводка —
  [v27/SUMMARY_real_eval.md](v27/SUMMARY_real_eval.md), карточка — [v27/TRAINING_REPORT_v27.md](v27/TRAINING_REPORT_v27.md).
...
- Путь v8→v11→v8→v16→**v27**: v16 объединил фикс тишины с реальными позитивами; v27 добавил
  short-негативы → закрыл «короткий захар». RepCNN/angular-2-ступень/radio-в-обучение — отброшены.
