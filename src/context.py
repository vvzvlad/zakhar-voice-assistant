...
idle longer than settings.context_ttl_seconds is considered finished and reset to
empty, so an unrelated later request starts fresh. Only the last
settings.context_max_turns user+assistant exchanges are kept.
"""
...
def _is_stale(context_path: str) -> bool:
    ...
    than settings.context_ttl_seconds is treated as finished.
    """
    try:
        age_seconds = datetime.now().timestamp() - os.path.getmtime(context_path)
    except OSError:
        return True
    return age_seconds > settings.context_ttl_seconds


def load_context(context_path: str) -> list[dict]:
    ...
    settings.context_max_turns * 2 messages.
    """
    if not os.path.exists(context_path) or _is_stale(context_path):
        return []
    ...
    return messages[-(settings.context_max_turns * 2):]
...
def append_context(context_path: str, user_text: str, assistant_text: str) -> None:
    ...
    settings.context_max_turns * 2 messages, and rewrites the whole file as JSONL.
    The full rewrite is intentional: it applies trimming and the staleness reset in
    one place. Failures are logged and never raised; the pipeline relies on that.
    """
    messages = load_context(context_path)
    messages.append({"role": "user", "content": user_text})
    messages.append({"role": "assistant", "content": assistant_text})
    messages = messages[-(settings.context_max_turns * 2):]
