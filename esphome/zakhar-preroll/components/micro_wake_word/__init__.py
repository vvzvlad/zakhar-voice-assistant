# ZAKHAR FORK of the stock ESPHome `micro_wake_word` component (copied VERBATIM from
# esphome 2026.5.3). The ONLY change vs. stock is in streaming_model.h/.cpp: a public,
# NON-destructive, gated peak-probability tap (set_stream_enabled / get_stream_enabled /
# read_and_reset_stream_peak) so the panel can show the live Wake Probability without
# coupling to detection. The Python config schema here is UNCHANGED — this file is the
# verbatim copy. Pinned to ESPHome 2026.5.x (same as the voice_assistant fork next to it);
# bumping ESPHome past 2026.5.x requires rebasing this fork (re-copy the stock component
# and re-apply the streaming_model patch). See ../voice_assistant for the same convention.
import hashlib
import json
import logging
