...
bool StreamingModel::perform_streaming_inference(const int8_t features[PREPROCESSOR_FEATURE_SIZE]) {
  ...
  if (this->loaded_) {
    ...
    if (this->current_stride_step_ >= stride) {
      ...
      // ZAKHAR FORK (additive). Wake Probability stream: keep a running max of the raw
      // per-inference probability (0..255) since the last main-loop read, so the 1s
      // panel sensor can report the PEAK over the interval. Gated so it costs nothing
      // when nobody is watching. Thread-safety: this runs on the inference FreeRTOS
      // task, while read_and_reset_stream_peak() runs on the main loop — hence
      // std::atomic (mirrors the existing cross-task vad_state_ pattern). VERIFY ON
      // REAL HARDWARE.
      if (this->stream_enabled_) {
        uint8_t newp = this->recent_streaming_probabilities_[this->last_n_index_];
        uint8_t prev = this->stream_peak_.load(std::memory_order_relaxed);
        while (newp > prev &&
               !this->stream_peak_.compare_exchange_weak(prev, newp, std::memory_order_relaxed)) {
        }
      }
    }
