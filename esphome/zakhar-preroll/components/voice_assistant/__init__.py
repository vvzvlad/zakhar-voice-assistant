...
# Pre-roll fork: how much microphone audio captured BEFORE voice_assistant.start is
# prepended to the outgoing stream. 0ms disables the feature (stock behavior).
CONF_PREROLL_DURATION = "preroll_duration"
CONF_PREROLL_SOURCE_1_ID = "preroll_source_1_id"
CONF_PREROLL_SOURCE_2_ID = "preroll_source_2_id"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_PREROLL_DURATION, default="0ms"): cv.All(
                cv.positive_time_period_milliseconds,
                cv.Range(max=cv.TimePeriod(milliseconds=5000)),
            ),
            cv.GenerateID(CONF_PREROLL_SOURCE_1_ID): cv.declare_id(
                microphone.MicrophoneSource
            ),
            cv.GenerateID(CONF_PREROLL_SOURCE_2_ID): cv.declare_id(
                microphone.MicrophoneSource
            ),
...
async def to_code(config):
    ...
    preroll_ms = config[CONF_PREROLL_DURATION].total_milliseconds
    if preroll_ms > 0:
        # Mirror each configured microphone source with a PASSIVE MicrophoneSource
        # (passive = never starts/stops the mic, only taps audio while something
        # else - e.g. micro_wake_word - keeps it running). These taps feed the
        # persistent ring buffers continuously, providing the pre-roll history.
        cg.add(var.set_preroll_duration(preroll_ms))
        preroll_ids = (CONF_PREROLL_SOURCE_1_ID, CONF_PREROLL_SOURCE_2_ID)
        preroll_setters = (var.set_preroll_source, var.set_preroll_source2)
        # VA supports at most two microphone sources (set_preroll_source/2 in C++).
        for i, mic_conf in enumerate(mic_sources[: len(preroll_ids)]):
            mic_var = await cg.get_variable(mic_conf[CONF_MICROPHONE])
            tap = cg.new_Pvariable(
                config[preroll_ids[i]],
                mic_var,
                mic_conf[CONF_BITS_PER_SAMPLE],
                mic_conf[CONF_GAIN_FACTOR],
                True,
            )
            for channel in mic_conf[CONF_CHANNELS]:
                cg.add(tap.add_channel(channel))
            cg.add(preroll_setters[i](tap))
