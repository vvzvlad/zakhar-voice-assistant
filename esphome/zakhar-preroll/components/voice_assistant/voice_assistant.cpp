...
static const uint8_t MAX_CHUNKS_PER_PASS = 8;  // 8 x 32 ms = 256 ms of audio per loop() pass

VoiceAssistant::VoiceAssistant() { global_voice_assistant = this; }

void VoiceAssistant::setup() {
  if (!this->preroll_active_()) {
    // Stock path: the session-owned microphone sources fill the ring buffers, which
    // only exist while a session is active.
    ...
    if (!this->allocate_buffers_()) {
      ESP_LOGE(TAG, "Pre-roll: failed to allocate persistent buffers");
      this->mark_failed();
      return;
    }
    ESP_LOGCONFIG(TAG, "Pre-roll enabled: %" PRIu32 " ms (%zu bytes/channel)", this->preroll_duration_ms_,
                  this->preroll_bytes_());
  }
...
bool VoiceAssistant::allocate_buffers_() {
  ...
  // Pre-roll fork: enlarge the ring buffers so they can hold the look-back audio on
  // top of the stock 512 ms of in-session margin. preroll_bytes_() is 0 in stock mode.
  const size_t ring_buffer_size = RING_BUFFER_SIZE + this->preroll_bytes_();
...
size_t VoiceAssistant::preroll_bytes_() const {
  if (!this->preroll_active_()) {
    return 0;
  }
  // 16 kHz, 16-bit mono per channel -> 32 bytes per millisecond
  return this->preroll_duration_ms_ * (SAMPLE_RATE_HZ / 1000) * sizeof(int16_t);
}

void VoiceAssistant::trim_ring_buffer_(const std::shared_ptr<ring_buffer::RingBuffer> &rb, uint8_t *scratch) {
  if (rb == nullptr) {
    return;
  }
  if (!this->preroll_active_() || !this->preroll_enabled_) {
    rb->reset();
    return;
  }
  if (scratch == nullptr) {
    // No scratch buffer to discard into: leave the buffer content untouched rather
    // than wiping the look-back audio. An over-full buffer self-trims anyway, since
    // RingBuffer::write() overwrites the oldest samples when full.
    return;
  }
  // Keep only the newest preroll_duration of audio: discard older samples by reading
  // them into the (scratch) send buffer. The discard target is computed once up front
  // so the loop is hard-bounded even if the microphone task keeps appending audio
  // while it runs - anything appended during the trim is newer than the kept window
  // and simply stays in the buffer. Keeping each discard sample-aligned (even byte
  // count) preserves int16 framing.
  const size_t keep = this->preroll_bytes_();
  const size_t available = rb->available();
  size_t to_discard = available > keep ? available - keep : 0;
  while (to_discard > 0) {
    size_t discard = std::min(to_discard, SEND_BUFFER_SIZE);
    discard &= ~static_cast<size_t>(1);
    if (discard == 0) {
      break;
    }
    size_t read_bytes = rb->read((void *) scratch, discard, 0);
    if (read_bytes == 0) {
      break;
    }
    to_discard -= std::min(read_bytes, to_discard);
  }
}

void VoiceAssistant::clear_buffers_() {
  // Pre-roll fork: instead of dropping everything, keep the newest preroll_duration of
  // audio queued so the upcoming stream starts with the look-back segment (and the
  // wake word itself) already in it.
  this->trim_ring_buffer_(this->ring_buffer_, this->send_buffer_);
  this->trim_ring_buffer_(this->ring_buffer2_, this->send_buffer2_);
...
void VoiceAssistant::loop() {
  ...
  switch (this->state_) {
    case State::IDLE: {
      ...
      } else if (!this->preroll_active_()) {
        // Pre-roll fork: the buffers are persistent (the passive taps keep filling
        // them between sessions), so they are only deallocated in stock mode.
    ...
    case State::STREAMING_MICROPHONE: {
      if (this->audio_mode_ == AUDIO_MODE_API) {
        ...
        // Pre-roll fork: a session may start with several seconds of audio already
        // buffered. Cap how much is drained per loop() pass so the API connection's
        // TX buffer is not flooded in one go; MAX_CHUNKS_PER_PASS chunks per pass
        // still drains far faster than real time.
        uint8_t chunks_sent = 0;

        while ((is_available || is_available2) && (chunks_sent++ < MAX_CHUNKS_PER_PASS)) {
