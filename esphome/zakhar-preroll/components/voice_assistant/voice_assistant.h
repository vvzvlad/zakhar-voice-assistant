...
class VoiceAssistant : public Component {
 public:
  ...
  // --- Pre-roll fork additions ---
  void set_preroll_duration(uint32_t ms) { this->preroll_duration_ms_ = ms; }
  void set_preroll_source(microphone::MicrophoneSource *source) { this->preroll_source_ = source; }
  void set_preroll_source2(microphone::MicrophoneSource *source) { this->preroll_source2_ = source; }
  /// Runtime switch (e.g. from a lambda): when disabled, sessions start with an
  /// empty buffer like stock firmware. Useful for manual captures that must not
  /// include audio from before the trigger.
  void set_preroll_enabled(bool enabled) { this->preroll_enabled_ = enabled; }
#ifdef USE_MICRO_WAKE_WORD
 ...
 protected:
  ...
  // --- Pre-roll fork additions ---
  bool preroll_active_() const { return this->preroll_duration_ms_ > 0 && this->preroll_source_ != nullptr; }
  size_t preroll_bytes_() const;
  /// Discard everything but the newest ``preroll_duration`` of audio (or all of it
  /// when pre-roll is disabled at runtime). Replaces ring_buffer->reset() so a new
  /// session starts with the look-back audio already queued for streaming. If no
  /// scratch buffer is available the content is left untouched (an over-full buffer
  /// self-trims via write-with-replacement).
  void trim_ring_buffer_(const std::shared_ptr<ring_buffer::RingBuffer> &rb, uint8_t *scratch);
  ...
  // Pre-roll fork: passive taps that feed the persistent ring buffers while the
  // microphone is kept running by another component (micro_wake_word on Voice PE).
  microphone::MicrophoneSource *preroll_source_{nullptr};
